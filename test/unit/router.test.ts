...
describe('Router', () => {
  describe('convenience methods', () => {
    ...
    for (const method of methods) {
      it(`should register and find ${method.toUpperCase()} route`, () => {
        ...
        (router as any)[method]('/test', handler);
  ...
  describe('group()', () => {
    ...
    it('should merge group middlewares with route middlewares', () => {
      ...
      router.group('/api', { middlewares: [groupMw as any] }, (group) => {
        group.get('/users', handler, { middlewares: [routeMw as any] });
      });
    ...
    it('should accumulate middlewares across nested groups', () => {
      ...
      router.group('/api', { middlewares: [mw1 as any] }, (api) => {
        api.group('/v1', { middlewares: [mw2 as any] }, (v1) => {
          v1.get('/users', handler, { middlewares: [mw3 as any] });
        });
      });

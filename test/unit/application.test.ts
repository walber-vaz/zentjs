...
import { Zent, zent } from '../../src/core/application';
...
describe('Application (Zent)', () => {
  ...
  describe('routing — convenience methods', () => {
    ...
    for (const method of methods) {
      it(`should register ${method.toUpperCase()} route and respond via inject`, async () => {
        const app = zent();
        (app as any)[method]('/test', (ctx: any) => {
        ...
        expect(res.json<any>().method).toBe(method.toUpperCase());
      });
    }
  });
  ...
  describe('use() — global middleware', () => {
    ...
    it('should throw TypeError for non-function middleware', () => {
      ...
      expect(() => app.use('not-fn' as any)).toThrow(TypeError);
    });
    ...
    it('should throw for invalid prefixed middleware signature', () => {
      ...
      expect(() => app.use('/api', 'not-fn' as any)).toThrow(
  ...
  describe('addHook() — lifecycle hooks', () => {
    ...
    it('should execute onError hook on error', async () => {
      ...
      app.addHook('onError', async (_ctx, err) => {
        capturedError = err;
      });
    ...
    it('should execute onSend hook and send transformed handler payload', async () => {
      ...
      app.addHook('onSend', async (_ctx, payload) => {
        return { ...(payload as any), transformed: true };
      });
  ...
  describe('setErrorHandler()', () => {
    it('should use custom error handler', async () => {
      ...
      app.setErrorHandler((error, ctx) => {
        ctx.res.status((error as any).statusCode || 500).json({
          custom: true,
    ...
    it('should return 404 for non-existent route', async () => {
      ...
      expect(res.json<any>().error).toBe('Not Found');
    });
  ...
  describe('setNotFoundHandler()', () => {
    ...
    it('should not intercept method not allowed errors', async () => {
      ...
      expect(res.statusCode).toBe(405);
      expect(res.json<any>().error).toBe('Method Not Allowed');
    });

    it('should throw TypeError for non-function not found handler', () => {
      const app = zent();

      expect(() => app.setNotFoundHandler('not-fn' as any)).toThrow(TypeError);
    });
  });

  describe('decorate()', () => {
    it('should add property to app instance', () => {
      const app = zent<any, any>();
    ...
    it('should be accessible via ctx.app', async () => {
      ...
      app.get('/version', (ctx) => {
        ctx.res.json({ version: (ctx.app as any).version });
      });
  ...
  describe('listen() and close()', () => {
    ...
    it('should invoke callback on listen', async () => {
      ...
      let cbAddress: string | undefined;
      await app.listen({ port: 0, host: '127.0.0.1' }, (_err, addr) => {
        cbAddress = addr;
      });
    ...
    it('should reject one of concurrent close() calls when server is already closing', async () => {
      ...
      expect(
        String((rejected as any).reason?.message || (rejected as any).reason)
      ).toMatch(/Server is not running/i);
    });
  ...
  describe('register() — plugins', () => {
    ...
    it('should throw for invalid scope.use signatures', async () => {
      ...
      app.register(async (scope) => {
        expect(() => scope.use('/api', 'not-fn' as any)).toThrow(
          'Invalid use() signature'
        );
        expect(() => scope.use(123 as any)).toThrow(
    ...
    it('should throw for invalid scope.addHook inputs', async () => {
      ...
      app.register(async (scope) => {
        expect(() =>
          scope.addHook('invalid-phase' as any, async () => {})
        ).toThrow('Invalid hook phase');
        expect(() => scope.addHook('onRequest', 'not-fn' as any)).toThrow(
    ...
    it('should allow plugin to set error handler via scope.setErrorHandler()', async () => {
      ...
      app.register(async (scope) => {
        scope.setErrorHandler((_err, ctx) => {
    ...
    it('should not leak decorators between sibling plugin scopes', async () => {
      ...
      app.register(
        async (scope) => {
          scope.get('/b', (ctx) => {
            ctx.res.json({ hasAOnly: (scope as any).hasDecorator('aOnly') });
          });
        },
        { prefix: '/scope-b' }
      );
